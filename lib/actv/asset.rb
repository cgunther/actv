...
module ACTV
  class Asset < ACTV::Identity
    ...
    def place
      @place ||= ACTV::Place.fetch_or_create(@attrs[:place]) unless @attrs[:place].nil?
    end

    def descriptions
      @descriptions ||= Array(@attrs[:assetDescriptions]).map do |description|
        ACTV::AssetDescription.fetch_or_create(description)
      end
    end
    ...
    def status
      @status ||= ACTV::AssetStatus.fetch_or_create(@attrs[:assetStatus]) unless @attrs[:assetStatus].nil?
    end
    alias asset_status status
    alias assetStatus status

    def legacy_data
      @legacy_data ||= ACTV::AssetLegacyData.fetch_or_create(@attrs[:assetLegacyData]) unless @attrs[:assetLegacyData].nil?
    end
    ...
    def channels
      @asset_channels ||= Array(@attrs[:assetChannels]).map do |channel|
        ACTV::AssetChannel.fetch_or_create(channel)
      end
    end
    ...
    def images
      @images ||= Array(@attrs[:assetImages]).map do |img|
        ACTV::AssetImage.fetch_or_create(img)
      end
    end
    ...
    def components
      @asset_components ||= Array(@attrs[:assetComponents]).map do |component|
        ACTV::AssetComponent.fetch_or_create(component)
      end
    end
    ...
    def prices
      @asset_prices ||= Array(@attrs[:assetPrices]).map do |price|
        ACTV::AssetPrice.fetch_or_create(price)
      end
    end
    ...
    def seo_urls
      @seo_urls ||= Array(@attrs[:assetSeoUrls]).map do |seo_url|
        ACTV::AssetSeoUrl.fetch_or_create(seo_url)
      end
    end
